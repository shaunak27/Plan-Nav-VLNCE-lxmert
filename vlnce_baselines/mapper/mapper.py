# Reference: https://github.com/facebookresearch/sound-spaces/blob/master/ss_baselines/av_wan/models/mapper.py
...
class Mapper(nn.Module):
    def __init__(self, gm_config, action_map_config):
        ...
        # both global map and local maps are square, the first channel is
        # obstacle map and the second is explored map
    ...
    def compute_navigable_xys(self):
        ...
        for angle in [0, 90, 180, 270]:
            ...
            for a, b in zip(self._navigable_xs, self._navigable_ys):
                c, d = transform_coordinates(
                    a, b, angle, self._geometric_map.shape[1], 
                    self._geometric_map.shape[0]
                )
    ...
    def update(self, prev_action: int, ego_map, intensity) -> Tuple[list, list]:
        ...
        # update global map
        rotated_geometric_map = rotate_map(
            self._geometric_map, -self._rotation, create_copy=False
        )
        rotated_x, rotated_y = transform_coordinates(
            self._x, self._y, -self._rotation, 
            self._geometric_map.shape[1], 
            self._geometric_map.shape[0]
        )
        ...
        def convert(a, b):
            return transform_coordinates(
                a, b, self._rotation, rotated_geometric_map.shape[1], 
                rotated_geometric_map.shape[0]
            )
    ...
    def get_maps_and_agent_pose(self):
        return self._geometric_map, self._x, self._y, self._orientation
    ...
    def global_to_egocentric(self, x, y):
        return self.allocentric_to_egocentric(
            x - self._x, y - self._y, self._action_map_res
        )

    def egocentric_to_global(self, delta_x, delta_y):
        allocentric = self.egocentric_to_allocentric(
            delta_x, delta_y, self._action_map_res
        )
    ...
    def get_egocentric_geometric_map(self):
        ...
        x, y = transform_coordinates(
            self._x, self._y, -self._rotation, self._geometric_map.shape[1], 
            self._geometric_map.shape[0]
        )
        map_size = rotated_geometric_map.shape[0]

        cropped_map = np.zeros(
            (self._gm_size, self._gm_size, self._geometric_map.shape[2])
        )
        ...
        if logging.root.level == logging.DEBUG:
            for j in range(size):
                for i in range(size):
                    ...
                    pg_x = int(i - size // 2)
                    pg_y = int(j - size // 2)
                    delta_x, delta_y = self.egocentric_to_allocentric(
                        pg_x, pg_y, action_map_res=action_map_res
                    )
