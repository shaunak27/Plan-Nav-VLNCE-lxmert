# From: https://github.com/facebookresearch/sound-spaces/blob/master/ss_baselines/av_wan/models/planner.py
#!/usr/bin/env python3
...
from vlnce_baselines.mapper.mapper import Mapper, to_array

class Planner:
    def __init__(self, task_config=None, model_dir=None, masking=True):
        self.mapper = Mapper(
    ...
    def update_map_and_graph(self, observation):
        ...
        geometric_map, x, y, orientation = self.mapper.get_maps_and_agent_pose()
        if not collided:
            non_navigable_points, blocked_paths = self.mapper.update(
                self._prev_action, ego_map, intensity
            )
    ...
    def plan(self, observation: dict, goal, stop, distribution=None) -> torch.Tensor:
        ...
        len_nodes = len(graph_nodes)
        if stop or (len_nodes == 2 and graph_nodes[0] == graph_nodes[1]):
            action = HabitatSimActions.STOP
            self._prev_next_node = None
        else:
            try:
                shortest_path = nx.shortest_path(
                    self._graph, source=graph_nodes[0], target=graph_nodes[1]
                )
