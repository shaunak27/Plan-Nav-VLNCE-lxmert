...
class InstructionEncoder(nn.Module):
    ...
    def forward(self, observations):
        ...
        # TODO address the instruction shape issue
        if len(instruction.shape) == 3:
            # This is PPO
            instruction = torch.reshape(
                instruction, (-1, self.config.max_length))

        lengths = (instruction != 0.0).long().sum(dim=1)
        lengths = np.int64(lengths.cpu())
