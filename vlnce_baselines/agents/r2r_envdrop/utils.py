...
class Tokenizer(object):
    ...
    def split_sentence(sentence):
        ...
        for word in [s.strip().lower() for s in
                     Tokenizer.SENTENCE_SPLIT_REGEX.split(sentence.strip()) if
                     len(s.strip()) > 0]:
    ...
    def shrink(self, inst):
        ...
        if len(inst) == 0:
            return inst
        end = np.argmax(np.array(inst) == self.word_to_index[
            '<EOS>'])  # If no <EOS>, return empty string
