...
from vlnce_baselines.encoders.simple_cnns import (
    SimpleDepthCNN, 
    SimpleRGBCNN
)
from vlnce_baselines.agents.policy import BasePolicy 

class GlobalSeq2SeqPolicy(BasePolicy):
    def __init__(
...
class Seq2SeqNet(Net):
    ...
    def __init__(self, observation_space: Space, model_config: Config, num_actions):
        ...
        self.reasoning_layer = nn.Linear(self.output_size,41)
        self._init_layers()
    ...
    def forward(self, observations, rnn_hidden_states, prev_actions, masks):
        ...
        if self.model_config.REASONING_LOSS.use and AuxLosses.is_active():
            object_hat = self.reasoning_layer(x)
            #print(observations["reasoning_sensor"])
            object_loss = F.cross_entropy(object_hat,observations["reasoning_sensor"].long())
            AuxLosses.register_loss(
                    "reasoning_layer",
                    object_loss,
                )
