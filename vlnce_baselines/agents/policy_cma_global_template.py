...
class GlobalCMATemplatePolicy(BasePolicy):
    def __init__(
...
class CMATemplateNet(Net):
    ...
    def __init__(self, observation_space: Space, model_config: Config, num_actions):
        ...
        self.template_encoder = InstructionEncoder(model_config.INSTRUCTION_ENCODER)
    ...
    def forward(self, observations, rnn_hidden_states, prev_actions, masks):
        ...
        template_embedding = self.template_encoder(observations)
        ...
        template_state_k = self.text_k(template_embedding)
        temp_mask = (template_embedding == 0.0).all(dim=1)
        temp_embedding = self._attn(
            text_state_q, template_state_k, template_embedding, temp_mask
        )
