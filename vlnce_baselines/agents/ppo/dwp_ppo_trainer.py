...
from vlnce_baselines.agents.policy_dwp import DWSeq2SeqPolicy
from vlnce_baselines.agents.ppo.ppo import PPO
from vlnce_baselines.common.utils import (
    transform_obs, 
    linear_decay, 
    exponential_decay,
...
@baseline_registry.register_trainer(name="dwp_ppo")
class PPOTrainer(BaseRLTrainer):
    ...
    def _setup_actor_critic_agent(
        self, ppo_cfg: Config, observation_space=None
    ) -> None:
        ...
        if observation_space is None:
            ...
            logger.info(f"Observation space:\n{observation_space}")
        
        self.actor_critic = DWSeq2SeqPolicy(
    ...
    def _collect_rollout_step(
        ...
        if "instruction" in observations[0]:
        ...
        # the current episode reward is added to the episode rewards only if the 
        # current episode is done the episode count will also increase by 1
    ...
    def train(self) -> None:
        ...
        # just a sanity check
        # plt.imshow(observations[0]['rgb']); plt.show()
        # plt.imshow(observations[0]['depth']); plt.show()
        # ego_map = observations[0]['ego_map']
        # plt.matshow(ego_map[:, :, 0]); plt.show()
        # plt.matshow(ego_map[:, :, 1]); plt.show()
        # geo_map = observations[0]['gm']
        # plt.matshow(geo_map[:, :, 0]); plt.show()
        # plt.matshow(geo_map[:, :, 1]); plt.show()
        # act_map = observations[0]['action_map']
        # plt.matshow(act_map); plt.show()
        # plt.close()
        ...
        # episode_rewards and episode_counts accumulates over the entire 
        # training course
        ...
        if ppo_cfg.use_linear_lr_decay:
            ...
            def lr_lambda(x):
                return exponential_decay(
                    x, self.config.NUM_UPDATES, ppo_cfg.exp_decay_lambda
                )
        else:
            def lr_lambda(x):
                return 1
        ...
        with TensorboardWriter(
            ...
            for update in range(start_update, self.config.NUM_UPDATES):
                ...
                # this reward is averaged over all the episodes happened during 
                # window_size updates approximately number of steps is 
                ...
                writer.add_scalar(
                    "Environment/SPL", 
                    deltas["spl"] / deltas["count"], 
                    count_steps
                )
                logging.debug(
                    'Number of steps: {}'.format(deltas["step"] / deltas["count"])
                )
                writer.add_scalar(
    ...
    def _eval_checkpoint(
        ...
        logger.info(f"on checkpoint: {checkpoint_path}, {checkpoint_index}")
        self.envs = construct_envs(
            ...
            for i in range(n_envs):
                ...
                if not_done_masks[i].item() == 0:
                    ...
                    if "top_down_map" in self.config.VISUALIZATION_OPTION:
                        ...
                        writer.add_image(
                            '{}_{}_{}/{}'.format(
                                config.EVAL.SPLIT, scene, current_episodes[i].episode_id,
        ...
        stats_file = os.path.join(
            config.TENSORBOARD_DIR, '{}_stats_{}.json'.format(
        ...
        logger.info(
            f"Average episode reward: {episode_reward_mean:.6f}")
        logger.info(
        ...
        if not config.EVAL.SPLIT.startswith('test'):
            writer.add_scalar(
                "{}/reward".format(config.EVAL.SPLIT), episode_reward_mean, 
                checkpoint_index
            )
            writer.add_scalar(
                "{}/reaching_waypoint".format(config.EVAL.SPLIT), 
            ...
            for metric_uuid in self.metric_uuids:
                writer.add_scalar(
                    ...
                    episode_metrics_mean[metric_uuid], checkpoint_index
                )
